	...
	if ( typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
		...
		factory(window['H'] = {
			...
			openFrameGroupNavOrFoot : function(callback, groupName, frames, index, headerSelector, footerSelector, options) {
				...
				that.fixStatusBar(function(offset) {
					...
					/*
					 ...
					 options = options || {};
					 var opt = that.extendObj(_options, options);
					 that.openFrameGroup(callback, groupName, frames, index, opt);
					 */
				}, headerSelector);
			},
			...
			dateTip : function(element, okCallback, cancelCallback) {
				...
				// 列表项目个数
				var yearItemSize = that.Ds(".H-select-item", that.D('.H-select-options:nth-child(1)')).length, monthItemSize = that.Ds(".H-select-item", that.D('.H-select-options:nth-child(2)')).length, dayItemSize = that.Ds(".H-select-item", that.D('.H-select-options:nth-child(3)')).length;
				// 当前索引
			...
			actionSheetTip2 : function(callback, title, buttons, cancelTitle, destructiveTitle, style) {
				var that = this;
				var o = {};
				...
				var html = '<div class="H-actionSheet H-display-none-important H-flexbox-vertical H-position-fixed H-vertical-top-0 H-horizontal-right-0 H-vertical-bottom-0 H-horizontal-left-0 H-height-100-percent H-width-100-percent H-background-color-transparent-3" style="z-index: 19923015; ">';
				html += '<div class="H-actionSheet-space H-flex-item"></div>';
				html += '<div class="H-actionSheet-list  animated slideInUp" style="-webkit-animation-duration:0.3s;animation-duration:0.3s;">';
				html += '<div class="H-actionSheet-title">';
				html += '<div class="H-actionSheet-item H-theme-background-color-white  H-text-show-row-1 H-border-vertical-bottom-after H-padding-10 H-center-all H-font-size-18 H-theme-font-color12">' + o.title + '</div>';
				html += '</div>';
				html += '<div class="H-actionSheet-buttons">';

				for (var i = 0; i < o.buttons.length; i++) {
					html += '<div class="H-actionSheet-item H-theme-background-color-white H-touch-active H-border-vertical-bottom-after H-text-show-row-1 H-padding-10 H-center-all H-font-size-16 H-theme-font-color1">' + o.buttons[i] + '</div>';
				}
			...
			promptTip : function(callback, title, msg, text, type, buttons) {
				...
				var _msg = '<div class="H-dialog-prompt-tip H-font-size-16" style="text-align:center">' + msg + '</div><input type="text" class="H-dialog-prompt-text H-width-100-percent H-border-none padding H-padding-10 H-box-sizing-border-box H-box-shadow-inset" style="border:1px solid #f1f1f1;text-align:center" placeholder="请输入值..." value="' + text + '" />';
			...
			toastLoading2 : function(duration) {
				var that = this;
