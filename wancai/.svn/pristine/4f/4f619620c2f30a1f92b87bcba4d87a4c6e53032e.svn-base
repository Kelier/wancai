 ...
 */
(function($) {
	/*tap event*/! function(a) {
		var b = {}, c = {};
		c.attachEvent = function(b, c, d) {
			return "addEventListener" in a ? b.addEventListener(c, d, !1) :
			void 0
		}, c.fireFakeEvent = function(a, b) {
			return document.createEvent ? a.target.dispatchEvent(c.createEvent(b)) :
			void 0
		...
		e = function(a) {
			...
			move : function(a) {
				return i.start || i.move ? ( a = c.getRealEvent(a), i.move = [a.pageX, a.pageY],
			...
			click : function(a) {
				return c.fireFakeEvent(a, b.options.eventName) ?
				void 0 : a.preventDefault()
			}
		}, f = function() {
			for (var a = 0; a < d.length; a++)
				if (d[a].test)
					return h = d[a].events, e("start"), e("move"), e("end"), !1;
		...
		p.prototype = {
			...
			_end : function(a) {
				if (this.enabled && d.eventType[a.type] === this.initiated) {
					...
					var e = f.round(this.x), k = f.round(this.y), h = f.abs(e - this.startX), g = f.abs(k - this.startY);
					b = 0;
					var l = "";
					...
					if (!this.resetPosition(this.options.bounceTime))
						if (this.scrollTo(e, k), this.moved)
							...
							this.options.tap && d.tap(a, this.options.tap), this.options.click && d.click(a), this._execEvent("scrollCancel")
				}
			},
			_resize : function() {
				var a = this;
			...
			scrollToElement : function(a, b, c, e, k) {
				if ( a = a.nodeType ? a : this.scroller.querySelector(a)) {
					...
					b =
					void 0 === b || null === b || "auto" === b ? f.max(f.abs(this.x - h.left), f.abs(this.y - h.top)) : b;
			...
			_wheel : function(a) {
				if (this.enabled) {
					...
					if ("deltaX" in a)
						1 === a.deltaMode ? ( b = -a.deltaX * this.options.mouseWheelSpeed, a = -a.deltaY * this.options.mouseWheelSpeed) : ( b = -a.deltaX, a = -a.deltaY);
					else if ("wheelDeltaX" in a)
						b = a.wheelDeltaX / 120 * this.options.mouseWheelSpeed, a = a.wheelDeltaY / 120 * this.options.mouseWheelSpeed;
			...
			goToPage : function(a, b, c, d) {
				...
				c =
				void 0 === c ? this.options.snapSpeed || f.max(f.max(f.min(f.abs(g - this.x), 1E3), f.min(f.abs(h - this.y), 1E3)), 300) : c;
				this.currentPage = {
					x : g,
					y : h,
					pageX : a,
			...
			_animate : function(a, b, c, e) {
				...
				var g = this, n = this.x, l = this.y, m = d.getTime(), p = m + c;
				this.isAnimating = !0;
				f()
			},
	...
	$.fn.mdatetimer = function(opts) {
		var defaults = {
			...
			years : [1969, 1970, 1971, 1972, 1973, 1974, 1975, 1976, 1977,1978,1979,
					...
					1990,1991,1992,1993,1994,1995,1996,1997,1998], //年份，数组或字符串'2000~2015'
			nowbtn : true,
			onOk : null,
			onCancel : null,
		};
		...
		var input = $(this), itemHeight = 40;
		var picker = {
			init : function() {
				var _this = this;

				_this.renderHTML();
				var container = $('.mt_poppanel'), mpYear = $('.mt_year', container), mpMonth = $('.mt_month', container), mpDay = $('.mt_day', container), mpHour = $('.mt_hour', container), mpMinute = $('.mt_minute', container), mpSecond = $('.mt_second', container);
				//初始化year

				var defaultDate = input.val() != '' ? F.getDateValue(input.val()) : new Date(), dYear = defaultDate.getFullYear(), dMonth = defaultDate.getMonth(), dDate = defaultDate.getDate(), dHour = defaultDate.getHours(), dMinute = defaultDate.getMinutes(), dSecond = defaultDate.getSeconds();
				...
				if (!option.nowbtn) {
					...
					$('.mt_cancel', container).css('border-right', '2px solid #DF6659');
				}
			...
			setValue : function(y, m, d, h, min, s) {
				var yearItem = $('.mt_year li[data-year="' + y + '"]'), monthItem = $('.mt_month li[data-month="' + m + '"]'), dayItem = $('.mt_day li[data-day="' + d + '"]'), hourItem = $('.mt_hour li[data-hour="' + h + '"]'), minuteItem = $('.mt_minute li[data-minute="' + min + '"]'), secondItem = $('.mt_second li[data-second="' + s + '"]');
				if (option.mode == 3) {
					if (h - 12 > 0) {
						s = 'pm';
